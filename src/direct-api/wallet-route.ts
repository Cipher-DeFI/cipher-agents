...
export const walletAnalysisRoute: Route = {
  ...
  handler: async (req: any, res: any, runtime: IAgentRuntime) => {
    try {
      ...
      const fumAction = actions.find(a => a.name === 'FUM_ANALYZE_WALLET');
      
      if (!fumAction) {
        return res.status(500).json({
          success: false,
          error: 'FUM_ANALYZE_WALLET action not found'
        });
      }
      ...
      await fumAction.handler(
        ...
        async (content: Content) => {
          ...
          actionName = (content as any).action || (content as any).actions?.[0] || 'FUM_ANALYZE_WALLET';
